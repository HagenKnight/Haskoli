...
{
    public abstract class ReadService<T, TQueryDTO, TKey, TEntity, TRepoRead, TContext> :
      IReadService<T, TQueryDTO, TKey, TEntity, TRepoRead, TContext>
      where T : class
      ...
      where TContext : DbContext, new()
    {
        ...
        {
            /*
              IEnumerable<TEntity> list = await _repository.AllAsyncInclude(orderBy, cancellationToken, includes);
                if (!string.IsNullOrWhiteSpace(fields))
                    list = list.AsQueryable().Select<TEntity>($"new({fields})");
